...
impl QwenCodeDriver {
    ...
    fn model_flag(model: &str) -> Option<String> {
        let stripped = model
            .strip_prefix("qwen-code/")
            .unwrap_or(model);
        match stripped {
            "qwen3-coder" => Some("qwen3-coder".to_string()),
            "qwen-coder-plus" => Some("qwen-coder-plus".to_string()),
            "qwq-32b" => Some("qwq-32b".to_string()),
...
impl LlmDriver for QwenCodeDriver {
    async fn complete(
        ...
        if !output.status.success() {
            ...
            {
                format!(
                    "Qwen Code CLI is not authenticated. Run: qwen --auth-type qwen-oauth\nDetail: {detail}"
                )
...
pub fn qwen_code_available() -> bool {
    QwenCodeDriver::detect().is_some()
        || qwen_credentials_exist()
}

/// Check if Qwen credentials exist.
///
/// Qwen Code stores session/credentials in `~/.qwen-code/` directory.
fn qwen_credentials_exist() -> bool {
    if let Some(home) = home_dir() {
        let qwen_dir = home.join(".qwen-code");
        qwen_dir.exists()
    } else {
        false
    }
...
mod tests {
    ...
    fn test_build_args_with_yolo() {
        let driver = QwenCodeDriver::new(None, true);
        let args = driver.build_args("hello", Some("qwen3-coder"), false);
    ...
    fn test_build_args_streaming() {
        ...
        assert!(!args.contains(&"json".to_string()) || args.contains(&"stream-json".to_string()));
    ...
    #[test]
    fn test_json_output_content_fallback() {
        let json = r#"{"content":"Fallback text"}"#;
        let parsed: QwenJsonOutput = serde_json::from_str(json).unwrap();
        assert!(parsed.result.is_none());
        assert_eq!(parsed.content.unwrap(), "Fallback text");
    }

    #[test]
    fn test_stream_event_parsing() {
        let json = r#"{"type":"content","content":"chunk"}"#;
        let event: QwenStreamEvent = serde_json::from_str(json).unwrap();
        assert_eq!(event.r#type, "content");
        assert_eq!(event.content.unwrap(), "chunk");
    }

    #[test]
    fn test_stream_event_result() {
        let json = r#"{"type":"result","result":"final answer","usage":{"input_tokens":100,"output_tokens":50}}"#;
        let event: QwenStreamEvent = serde_json::from_str(json).unwrap();
